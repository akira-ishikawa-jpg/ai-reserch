...
const MusicDataClass = preload("res://scripts/data/music_data.gd")
...
static func generate_all_se() -> Dictionary:
	var result = {}
	for name in MusicDataClass.get_all_se_names():
...
static func _render_song(song) -> AudioStreamWAV:
...
static func _render_track(track, beat_duration: float, total_samples: int) -> PackedFloat32Array:
	var buffer = PackedFloat32Array()
	buffer.resize(total_samples)
	buffer.fill(0.0)

	var sample_pos = 0
	var timbre = track.timbre
...
## 単一ノートの波形合成
static func _synthesize_note(freq: float, duration: float, timbre, velocity: float) -> PackedFloat32Array:
...
static func _generate_wave(wave_type: int, t: float, freq: float) -> float:
	...
	match wave_type:
		MusicDataClass.WaveType.SINE:
			return sin(TAU * phase)
		MusicDataClass.WaveType.SQUARE:
			return 1.0 if phase < 0.5 else -1.0
		MusicDataClass.WaveType.TRIANGLE:
			if phase < 0.25:
				return phase * 4.0
			elif phase < 0.75:
...
static func _calculate_envelope(t: float, duration: float, env) -> float:
	var a = env.attack
	var d = env.decay
	var s = env.sustain
	var r = env.release
...
static func _render_se(se) -> AudioStreamWAV:
...
static func _render_se_layer(layer, duration: float, total_samples: int) -> PackedFloat32Array:
	var buffer = PackedFloat32Array()
	buffer.resize(total_samples)
	...
	for i in range(total_samples):
		...
		if layer.noise_mix >= 1.0:
			sample = _generate_wave(MusicDataClass.WaveType.NOISE, t, freq)
		elif layer.noise_mix > 0.0:
			var wave = _generate_wave(layer.wave, t, freq)
			var noise = _generate_wave(MusicDataClass.WaveType.NOISE, t, freq)
...
static func _get_frequency(note_name: String) -> float:
	if MusicDataClass.NOTE_FREQ.has(note_name):
		return MusicDataClass.NOTE_FREQ[note_name]
